...
Population_Estimates = read_excel("~/github/geog-13-labs/data/PopulationEstimates.xls")
...
## I'm afraid my experience with weighted mean centers is zero and I'm not really sure
## how the summation is supposed to work. Hopefully I can get some guidance about how
## they work after class and finish this.
```{r}
#Extra credit attempt: weighted mean center

#import spatial data
County_Centroids <- read_csv("~/github/geog-13-labs/data/county-centroids.csv")

Land_Data_States <- read.csv("~/github/geog-13-labs/data/landdata-states.csv")
...
Geo_COVID_Data <- COVID %>% 
  ...
  ungroup() %>% 
  filter(new_cases != "NA") %>% 
  mutate(monthly_cases_total = sum(new_cases, na.rm = TRUE)) %>% 
  group_by(date) %>% 
  mutate(daily_cases_total = sum(cases, na.rm = TRUE)) %>% 
  summarise(weighted_mean_X = sum(LON * new_cases, na.rm = TRUE) / daily_cases_total, 
            weighted_mean_Y = sum(LAT * new_cases, na.rm = TRUE) / daily_cases_total,
            daily_cases_total = daily_cases_total,
            monthy_cases_total = monthly_cases_total) %>% 
  ungroup() %>% 
  ggplot(aes(x = weighted_mean_X, y = weighted_mean_Y)) +
  geom_point(col = "red") +
